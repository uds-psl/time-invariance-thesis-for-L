...
Section Rev.
  ...
  Definition Rev_Step_steps {sigX X} {cX : codable sigX X} (xs : list X) : nat :=
    match xs with
  ...
  Fixpoint Rev_Append_steps {sigX X} {cX : codable sigX X} (xs : list X) : nat :=
    match xs with
  ...
  Definition Rev_steps {sigX X} {cX : codable sigX X} (xs : list X) := 1 + Constr_nil_steps + Rev_Append_steps xs.
